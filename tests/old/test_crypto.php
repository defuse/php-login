<?php
/*
 * This code is hereby placed into the public domain by its author Taylor 
 * Hornby. It may be freely used for any purpose whatsoever.
 *
 * Developer's Contact Information:
 * WWW:     https://defuse.ca/
 * EMAIL:   <fillmein>
 * 
 * ***DISCLAIMER***
 *
 * ALL PUBLIC DOMAIN MATERIAL IS OFFERED AS-IS. NO REPRESENTATIONS OR
 * WARRANTIES OF ANY KIND ARE MADE CONCERNING THE MATERIALS, EXPRESS,
 * IMPLIED, STATUTORY OR OTHERWISE, INCLUDING, WITHOUT LIMITATION,
 * WARRANTIES OF TITLE, MERCHANTIBILITY, FITNESS FOR A PARTICULAR PURPOSE,
 * NONINFRINGEMENT, OR THE ABSENCE OF LATENT OR OTHER DEFECTS, ACCURACY, OR
 * THE PRESENCE OF ABSENCE OF ERRORS, WHETHER OR NOT DISCOVERABLE.
 * 
 * Limitation on Liability.
 * 
 * IN NO EVENT WILL THE AUTHOR(S), PUBLISHER(S), OR PRESENTER(S) OF ANY
 * PUBLIC DOMAIN MATERIAL BE LIABLE TO YOU ON ANY LEGAL THEORY FOR ANY
 * SPECIAL, INCIDENTAL, CONSEQUENTIAL, PUNITIVE OR EXEMPLARY DAMAGES
 * ARISING OUT OF THIS LICENSE OR THE USE OF THE WORK, EVEN IF THE
 * AUTHOR(S), PUBLISHER(S), OR PRESENTER(S) HAVE BEEN ADVISED OF THE
 * POSSIBILITY OF SUCH DAMAGES.
*/


//TODO: Test EVERY function
//      - make sure they return correct key sizes, not in hex, etc.

require_once('inc/security.conf.php');
require_once('security/Crypto.php');
require_once('tests/old/testlib.php');

set_time_limit(300);

// Test encrypt and decrypt are inverse functions
$plaintext = "applesauce";
$key = "apple";
$ct = Crypto::Encrypt($plaintext, $key);
$pt = Crypto::Decrypt($ct, $key);
test_assert($plaintext === $pt, "Encrypt and Decrypt");

// Test invalid key does not decrypt data
$badkey = "apples";
test_assert(Crypto::Decrypt($ct, $badkey) === FALSE, "Bad key.");

// Test altered HMAC results in failed decryption
$hmac_manip = $ct;
$hmac_manip[0] = chr((ord($hmac_manip[0]) + 1) % 256);
test_assert(Crypto::Decrypt($hmac_manip, $key) === FALSE, "HMAC manipulation.");

// Test altered IV results in failed decryption
$hmacsize = strlen(hash_hmac(SERVERSIDE_HASH_ALGORITHM, '', '', true));
$iv_manip = $ct;
$iv_manip[$hmacsize+1] = chr((ord($iv_manip[$hmacsize+1]) + 1) % 256);
test_assert(Crypto::Decrypt($iv_manip, $key) === FALSE, "IV manipulation.");

// Test altered ciphertext results in failed decryption
$ct_manip = $ct;
$ct_manip[$hmacsize+1] = chr((ord($ct_manip[$hmacsize+1]) + 1) % 256);
test_assert(Crypto::Decrypt($ct_manip, $key) === FALSE, "CT manipulation.");

// Test that the same plaintext doesn't encrypt to the same ciphertext.
$plaintext = "";
$key = "";
$a = Crypto::Encrypt($plaintext, $key);
$b = Crypto::Encrypt($plaintext, $key);
test_assert($a != $b, "Unique encryption.");


// ========== Test PBKDF2 ========== 
//Test vector source: https://www.ietf.org/rfc/rfc6070.txt

$pbkdf_vectors = array(
    array(
        'algorithm' => 'sha1', 
        'password' => "password", 
        'salt' => "salt", 
        'iterations' => 1, 
        'keylength' => 20, 
        'output' => "0c60c80f961f0e71f3a9b524af6012062fe037a6" 
        ),
    array(
        'algorithm' => 'sha1', 
        'password' => "password", 
        'salt' => "salt", 
        'iterations' => 2, 
        'keylength' => 20, 
        'output' => "ea6c014dc72d6f8ccd1ed92ace1d41f0d8de8957"
        ),
    array(
        'algorithm' => 'sha1', 
        'password' => "password", 
        'salt' => "salt", 
        'iterations' => 4096, 
        'keylength' => 20, 
        'output' => "4b007901b765489abead49d926f721d065a429c1"
        ),
    array(
        'algorithm' => 'sha1', 
        'password' => "password", 
        'salt' => "salt", 
        'iterations' => 7613, 
        'keylength' => 769, 
        'output' => ""
        ),
    array(
        'algorithm' => 'sha1', 
        'password' => "passwordPASSWORDpassword", 
        'salt' => "saltSALTsaltSALTsaltSALTsaltSALTsalt", 
        'iterations' => 4096, 
        'keylength' => 25, 
        'output' => "3d2eec4fe41c849b80c8d83662c0e44a8b291a964cf2f07038"
        ), 
    array(
        'algorithm' => 'sha1', 
        'password' => "pass\0word", 
        'salt' => "sa\0lt", 
        'iterations' => 4096, 
        'keylength' => 16, 
        'output' => "56fa6aa75548099dcc37d7f03425e0c3"
        ),            
);

foreach($pbkdf_vectors as $test)
{
    $realOut = bin2hex(Crypto::pbkdf2($test['algorithm'], $test['password'], $test['salt'], 
                                                    $test['iterations'], $test['keylength']));

    test_assert($realOut === $test['output'], "PBKDF Vector");
    if($realOut !== $test['output'])
    {
        echo "FAIL: PBKDF VECTOR:\n";
        print_r($test);
        echo "\n\nGot: $realOut\n\n";
    }
}

// ========== Test password hashing functions ========== 

$hash = Crypto::HashPassword("password");
test_assert(Crypto::ValidatePassword("password", $hash), "Correct password");

$hash = Crypto::HashPassword("apple");
test_assert(!Crypto::ValidatePassword("orange", $hash), "Different passwords");


$hash = Crypto::HashPassword("password");
$hash[0] = "z";
test_assert(!Crypto::ValidatePassword("password", $hash), "Salt modification");


$hash = Crypto::HashPassword("password");
$hash[strlen($hash) - 1] = "z";
test_assert(!Crypto::ValidatePassword("password", $hash), "Hash modification");


$hash1 = Crypto::HashPassword("password");
$hash2 = Crypto::HashPassword("password");
test_assert($hash1 != $hash2, "Overall hash not equal.");
test_assert(substr($hash1, 0, 2 * SERVERSIDE_SALT_OCTETS) != substr($hash2, 0, 2 * SERVERSIDE_SALT_OCTETS), "Salt part not equal.");
test_assert(substr($hash1, 2 * SERVERSIDE_SALT_OCTETS, 2 * SERVERSIDE_HASH_OCTETS) != substr($hash2, 2 * SERVERSIDE_SALT_OCTETS, 2 * SERVERSIDE_HASH_OCTETS), "Hash part not equal.");

$key1 = Crypto::CreateKey("password", "testing");
$key2 = Crypto::CreateKey("password", "testing");
test_assert($key1 == $key2, "CreateKey same everything.");

$key1 = Crypto::CreateKey("password", "testingONE");
$key2 = Crypto::CreateKey("password", "testingTWO");
test_assert($key1 != $key2, "CreateKey different purpose.");

$key1 = Crypto::CreateKey("passwordONE", "testing");
$key2 = Crypto::CreateKey("passwordTWO", "testing");
test_assert($key1 != $key2, "CreateKey different password.");


test_allpass();
?>
